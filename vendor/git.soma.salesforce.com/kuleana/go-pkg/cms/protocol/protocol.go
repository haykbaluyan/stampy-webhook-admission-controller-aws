...
var logger = xlog.NewPackageLogger("git.soma.salesforce.com/kuleana/go-pkg", "cms/proto")
...
var (
	...
	ErrNoCertificate = errors.New("unable to find certificate")
...
var oidIDC = asn1.ObjectIdentifier{1, 3, 6, 1, 4, 1, 311, 2, 1, 4}
...
func (eci EncapsulatedContentInfo) EContentValue() ([]byte, error) {
	...
	// Special case for signed PE/COFF
	if eci.EContentType.Equal(oidIDC) {
		var signed asn1.RawValue
		if _, err := asn1.Unmarshal(eci.EContent.Bytes, &signed); err != nil {
...
func (si SignerInfo) FindCertificate(certs []*x509.Certificate) (*x509.Certificate, error) {
	...
	case 1: // SID is issuer and serial number
		...
		if logger.LevelAt(xlog.DEBUG) {
			...
			if _, err := asn1.Unmarshal(isn.Issuer.FullBytes, &issuer); err == nil {
				logger.Debugf("api=FindCertificate, reason=not_found, version=1, serial=%x, issuer='%s'",
					isn.SerialNumber, issuer.String())
			}
		}
...
func (si SignerInfo) String() string {
	...
	case 1: // SID is issuer and serial number
		...
		var issuer pkix.RDNSequence
		_, err = asn1.Unmarshal(isn.Issuer.FullBytes, &issuer)
		if err == nil {
			return fmt.Sprintf("Serial: %x, Issuer: %s", isn.SerialNumber, issuer.String())
		}
	case 3: // SID is SubjectKeyIdentifier
		ski, err := si.subjectKeyIdentifierSID()
		if err == nil {
			return fmt.Sprintf("Subject Key Identifier=%x", ski)
		}
	}
	return ""
}
